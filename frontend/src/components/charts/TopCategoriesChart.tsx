...
import { fetchTopCategories } from '../../api/client';
import type { TopCategoryPoint } from '../../api/client';
    ...
    <div className="rounded-lg px-4 py-3 shadow-lg"
      ...
      {item.esOtros && item.count !== undefined && (
        <p className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>
          {item.count} categorías agrupadas
        </p>
      )}
...
function LabelPorcion(props: PieLabelRenderProps) {
  ...
  const RADIO_LINEA_START = outerRadius + 6;
  const RADIO_LINEA_END   = outerRadius + 18;
  const OFFSET_TEXTO      = 4;
...
export default function TopCategoriesChart({ empkey, ubicod, timeRange, refreshKey }: Props) {
  const { colors } = useTheme();
  const { data: raw, loading, error } = useFetchChartData(
    (signal) => fetchTopCategories({ empkey, ubicod, from: String(timeRange.from), to: String(timeRange.to), signal }),
    [empkey, ubicod, timeRange.from, timeRange.to, refreshKey],
    [] as TopCategoryPoint[],
  );
  ...
  const { totalGeneral, chartData } = useMemo(() => {
    ...
    for (const p of raw) {
      if (visiblesCount >= MAX_SLICES) break;
      visibles.push({ nombre: p.categoria, total: p.total });
      visiblesCount++;
      acumulado += p.total;
      if (total > 0 && acumulado / total >= COBERTURA_OBJETIVO) break;
    }

    const desbordados = raw.slice(visiblesCount);
    if (desbordados.length > 0) {
      visibles.push({
        nombre:  `Otros (${desbordados.length} categorías)`,
